...
def create_model(norm=True, ema=False):
    ...
    net = net_factory('mcnetv2',1,1)  # 'unet','enet','pnet'
    if norm:
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            outputs_aux1, outputs_aux2, outputs_aux3 = model(imgs_all)
            preds_aux1, _ = outputs_aux1.split([imgs.shape[0], un_w.shape[0]])
            preds_aux2, _ = outputs_aux2.split([imgs.shape[0], un_w.shape[0]])
            preds_aux3, _ = outputs_aux3.split([imgs.shape[0], un_w.shape[0]])
            soft_outputs_aux1 = torch.sigmoid(outputs_aux1).ge(0.5).to(torch.float32)
            soft_outputs_aux2 = torch.sigmoid(outputs_aux2).ge(0.5).to(torch.float32)
            soft_outputs_aux3 = torch.sigmoid(outputs_aux3).ge(0.5).to(torch.float32)
            del imgs_all, outputs_aux1, outputs_aux2, outputs_aux3
            """
            1.1 Supervised Loss
            """
            ...
            loss_su = (loss_su_1 + loss_su_2 + loss_su_3) / 3.0
            su_loss = su_loss + (loss_su.detach().clone().item() / imgs.shape[0])
            """
            2. Unsupervised Loss
            """
            cons_weight = get_current_consistency_weight(iter_num)
            cons_loss2_1 = loss_u(soft_outputs_aux2, sharpening(soft_outputs_aux1.detach()))
            cons_loss3_1 = loss_u(soft_outputs_aux3, sharpening(soft_outputs_aux1.detach()))
            
            cons_loss1_2 = loss_u(soft_outputs_aux1, sharpening(soft_outputs_aux2.detach()))
            cons_loss3_2 = loss_u(soft_outputs_aux3, sharpening(soft_outputs_aux2.detach()))
            
            cons_loss1_3 = loss_u(soft_outputs_aux1, sharpening(soft_outputs_aux3.detach()))
            cons_loss2_3 = loss_u(soft_outputs_aux2, sharpening(soft_outputs_aux3.detach()))
            
            cons_loss = (cons_loss2_1 + cons_loss3_1 + cons_loss1_2 + cons_loss3_2 + cons_loss1_3 + cons_loss2_3) / 6.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/MCNet2"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/MCNet2"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/MCNet2"))
