...
def create_model(norm=False, ema=False):
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            """
            1. Labeled data (imgs, labels) and Unlabeled data preds
            """
            imgs_all = torch.cat((imgs, un_w))
            preds_t_all = model_t(imgs_all)
            preds_s_all = model_s(imgs_all)
            preds_t, _ = preds_t_all.split([imgs.shape[0], un_w.shape[0]])
            preds_s, _ = preds_s_all.split([imgs.shape[0], un_w.shape[0]])
            """
            1.1. Supervised Loss
            """
            loss_su_1 = loss_l(preds_t, labels)
            loss_su_2 = loss_l(preds_s, labels)
            loss_su = loss_su_1 + loss_su_2
            su_loss = su_loss + (loss_su.detach().clone().item() / (imgs.shape[0])*2)
            """
            1.2. CPS Loss
            """
            mask1 = (torch.sigmoid(preds_t_all).ge(threshold)).to(dtype=torch.float32).detach()
            mask2 = (torch.sigmoid(preds_s_all).ge(threshold)).to(dtype=torch.float32).detach()
            loss_u_1 = loss_u(torch.sigmoid(preds_t_all), mask2)
            loss_u_2 = loss_u(torch.sigmoid(preds_s_all), mask1)
            cps_loss = loss_u_1 + loss_u_2
            un_loss = un_loss + (cps_loss.detach().clone().item() / un_batch_size)
            """
            2. Loss Backward
            """
            cons_weight = get_current_consistency_weight(iter_num)
            loss = loss_su + (cons_weight * cps_loss)
...
def main(args):
    ...
    print("=" * 50)
    loss = DiceBCELoss()
    loss_un = nn.BCEWithLogitsLoss()
    optimizer_t = torch.optim.AdamW(params=model_t.parameters(), lr=args.lr)
    scheduler_t = CosineAnnealingLR(optimizer=optimizer_t, T_max=args.e)
    ...
    scheduler_s = CosineAnnealingLR(optimizer=optimizer_s, T_max=args.e)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/400_8952/CPS_resnest101"))
    parser.add_argument("--t_txt_path", "--train_txt_path", default="./data/cag/labeled_400_2.txt") # labeled_400_2
    parser.add_argument("--t_un_txt_path", "--train_un_txt_path", default="./data/cag/unlabeled_all.txt") # unlabeled_all
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/CPS"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/CPS"))
