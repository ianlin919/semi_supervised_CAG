...
def create_model(norm=True, ema=False):
    ...
    net = net_factory('unet_dtc',1,1)  # 'unet','enet','pnet'
    if norm:
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            output, output_tanh = model(imgs_all)
            out_l, _ = output.split([imgs.shape[0], un_batch_size])
            out_l_tanh, _ = output_tanh.split([imgs.shape[0], un_batch_size])
            """
            1.1 distance
            """
            with torch.no_grad():
                ...
                gt_dis = f(labels)
            """
            1.2 labeled data loss
            """
            loss_sdf = loss_u(out_l_tanh, gt_dis)
            loss_seg_dice = loss_l(out_l, labels)
            su_loss = su_loss + (loss_seg_dice.detach().clone().item() / imgs.shape[0])
            """
            1.3 consistency loss
            """
            dis_to_mask = torch.sigmoid(-output_tanh) * torch.sigmoid(-output_tanh).ge(0.5).float()
            consistency_loss = loss_u(dis_to_mask, torch.sigmoid(output)* torch.sigmoid(output).ge(0.5).float())
            un_loss = un_loss + (consistency_loss.detach().clone().item() / (imgs.shape[0] + un_batch_size))
            """
            1.4 loss backward
            """            
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/DTC"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/DTC"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/10_90/DTC"))
