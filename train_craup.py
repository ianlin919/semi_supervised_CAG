...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, ada_th, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), (un_w, un_s) in zip(cycle(train_loader_l), train_loader_u):
            ...
            cons_loss = loss_u((torch.sigmoid(pred_s) * mask), (torch.sigmoid(pred_w.detach()) * mask)) # test2 best
            ...
            # un_w_a = applier_s(un_w.detach().clone())
            # s_loss_u = loss_l(model_s(un_w_a), pseudo_label)
            # ====================================
            # s_loss = s_loss_u + cons_loss_vat
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/CRAUP"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/CRAUP_r50"))
