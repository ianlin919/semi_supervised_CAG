...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, ada_th):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), (un_w, un_s) in zip(cycle(train_loader_l), train_loader_u):
            ...
            # original----------------------------
            mask = (torch.sigmoid(pred_w).ge(threshold)).to(dtype=torch.float32)
            cons_loss = cons_weight * loss_u(torch.sigmoid(pred_s), (torch.sigmoid(pred_w.detach()) * mask))
            # best--------------------------------
            ...
            """
            """
            2.2. Student unlabeled data
            """
            mask = (torch.sigmoid(pred_w.detach()).ge(threshold)).to(dtype=torch.float32)
            s_loss_u = loss_l(model_s(un_w), mask)
            # ====================================
            fabric.backward(s_loss_u)
            optimizer_s.step()
            # ====================================
            iter_num = iter_num + 1
...
def main(args):
    ...
    for epoch in range(args.e):
        ...
        print("Best F1/DSC {} on Epoch {} / th {}".format(str(best_target),str(best_epoch), str(ada_th.confidence))) #ada_th.confidence
...
if __name__ == "__main__":
    ...
    parser.add_argument("--i_dir", "--img_dir", default=Path("./data/cag/imgs2"))
    parser.add_argument("--l_dir", "--label_dir", default=Path("./data/cag/labels2"))
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/test_th_ada"))
    parser.add_argument("--t_txt_path", "--train_txt_path", default="./data/cag/labeled_20_2.txt") # labeled_400_2 labeled_771_2
    parser.add_argument("--t_un_txt_path", "--train_un_txt_path", default="./data/cag/un_500.txt") # unlabeled_all un_500
