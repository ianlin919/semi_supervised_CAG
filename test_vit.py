...
model2 = ViT_seg(config, img_size=args.patch_size, num_classes=1).cuda()
model2.load_from(config)

def count_params(model):
    return print(sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6)
import torch
...
net = net_factory('unet',1,1)
model2 = net.eval().cuda()
s_time = time()
for i in range(100):
    x = torch.rand(1,1,224,224).cuda()
    output = model2(x)
