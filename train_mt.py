...
def train(fabric, model_, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            3. Unsupervised Loss
            """
            loss_un = loss_u(torch.sigmoid(out_w), torch.sigmoid(ema_output))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG//F2/20_500/MT")) 
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/MT"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/MT"))
    ...
    import shutil
    # shutil.copytree('./', args.s_dir/'code', shutil.ignore_patterns('*.pth', 'data', 'logs'))
