...
def train(fabric, model_, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            """
            1. ICT mix factors and Mix images 
            """
            ict_mix_factors = np.random.beta(0.2, 0.2, size=(un_batch_size//2, 1, 1, 1))
            ict_mix_factors = torch.tensor(ict_mix_factors, dtype=torch.float).to(fabric.device)
            un_w_0 = un_w[0:un_batch_size//2]
            un_w_1 = un_w[un_batch_size//2:]
            batch_ux_mixed = un_w_0 * (1.0 - ict_mix_factors) + un_w_1 * ict_mix_factors
            """
            1.1 teacher preds
            """
            imgs_all = torch.cat((imgs, batch_ux_mixed))
            output = model(imgs_all)
            out_l, out_w_mix = output.split([imgs.shape[0], batch_ux_mixed.shape[0]])
            """
            1.2 student pred
            """
            with torch.no_grad():
                ema_output_0 = model_ema(un_w_0)
                ema_output_1 = model_ema(un_w_1)
                ema_pred_mixed = ema_output_0 * (1.0 - ict_mix_factors) + ema_output_1 * ict_mix_factors
            """
            2. Supervised Loss
            """
            ...
            3. Unsupervised Loss
            """
            loss_un = loss_u(torch.sigmoid(out_w_mix), torch.sigmoid(ema_pred_mixed))
            un_loss = un_loss + (loss_un.detach().clone().item() / out_w_mix.shape[0])
            
            cons_weight = get_current_consistency_weight(iter_num)
            loss_ = loss_su + cons_weight * loss_un
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/ICT"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/ICT"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/ICT"))
