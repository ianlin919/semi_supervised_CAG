...
def create_model(norm=True, ema=False):
    ...
    net = net_factory('mcnetv1',1,1)  # 'unet','enet','pnet'
    if norm:
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            outputs_aux1, outputs_aux2 = model(imgs_all)
            ...
            del imgs_all, outputs_aux1, outputs_aux2
            """
            1.1 Supervised Loss
            """
            ...
            cons_loss = (cons_loss2_1 + cons_loss1_2) / 2.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/MCNet"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/MCNet"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/MCNet"))
