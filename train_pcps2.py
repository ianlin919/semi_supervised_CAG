...
def create_model(norm=True, ema=False):
    ...
    net = net_factory('pcps2',1,1)  # 'unet','enet','pnet'
    if norm:
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            """
            aux_names = list(set(['FeatureNoise', 'FeatureDropout', 'Dropout', 'VAT', 'None']))
            aux_name_1, aux_name_2 = random.sample(aux_names, k=2)
            imgs_all = torch.cat((imgs, un_w))
            outputs1, outputs_aux1 = model1(imgs_all, aux_name_1)
            outputs2, outputs_aux2 = model2(imgs_all, aux_name_2)
            preds_1, preds_w1 = outputs1.split([imgs.shape[0], un_w.shape[0]])
            preds_2, preds_w2 = outputs2.split([imgs.shape[0], un_w.shape[0]])
            preds_aux_1, preds_aux_w1 = outputs_aux1.split([imgs.shape[0], un_w.shape[0]])
            preds_aux_2, preds_aux_w2= outputs_aux2.split([imgs.shape[0], un_w.shape[0]])
            del preds_aux_1, preds_aux_2
            """
            1.1 Supervised Loss
            """
            loss_su_1 = loss_l(preds_1, labels)
            loss_su_2 = loss_l(preds_2, labels)
            loss_su = (loss_su_1 + loss_su_2) / 2.0
            su_loss = su_loss + (loss_su.detach().clone().item() / imgs.shape[0])
            """
            2. Unsupervised Loss
            """
            cons_weight = get_current_consistency_weight(iter_num)
            mask_1 = torch.sigmoid(outputs1).ge(0.5).to(torch.float32).detach()
            mask_2 = torch.sigmoid(outputs2).ge(0.5).to(torch.float32).detach()
            cons_loss1 = loss_u(torch.sigmoid(outputs_aux2), mask_1)
            cons_loss2 = loss_u(torch.sigmoid(outputs_aux1), mask_2)
            
            if aux_name_1 == 'None':
                preds_w1 = sharpening(preds_w1)
            if aux_name_2 == 'None':
                preds_w2 = sharpening(preds_w2)
            cons_loss3 = loss_u(preds_aux_w1, preds_w1)
            cons_loss4 = loss_u(preds_aux_w2, preds_w2)
            
            cons_loss = (cons_loss1 + cons_loss2 + cons_loss3 + cons_loss4) / 4.0
...
def main(args):
    ...
    model2 = create_model()
    print("=" * 50)
    print("model parameters: {:.2f}M".format(sum(p.numel() for p in model1.parameters())/1e6))
    print("model parameters: {:.2f}M".format(sum(p.numel() for p in model2.parameters())/1e6))
    print("=" * 50)
    loss = DiceBCELoss()
    loss_un = nn.MSELoss()
    optimizer1 = torch.optim.AdamW(params=model1.parameters(), lr=args.lr)
    scheduler1 = CosineAnnealingLR(optimizer=optimizer1, T_max=args.e, eta_min=1e-5)
    optimizer2 = torch.optim.AdamW(params=model2.parameters(), lr=args.lr)
    scheduler2 = CosineAnnealingLR(optimizer=optimizer2, T_max=args.e, eta_min=1e-5)
    model1, optimizer1 = fabric.setup(model1, optimizer1)
    model2, optimizer2 = fabric.setup(model2, optimizer2)
    """
    ...
    for epoch in range(args.e):
        train_history = train(fabric, [model1, model2], [train_loader, train_loader_u], 
                              [loss, loss_un], [optimizer1, optimizer2], [scheduler1, scheduler2], train_history, epoch, args)
        train_history, current_target = valid(fabric, model1, valid_loader, train_history)
        best_target, best_epoch = save_model(fabric, model1, best_target, current_target, args.s_dir, epoch, best_epoch)
    ...
    test(fabric, model1, valid_loader, args.s_dir)
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/PCPS2"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/PCPS2"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/20_80/PCPS2"))
    # parser.add_argument("--t_txt_path", "--train_txt_path", default="./data/DCA1/DCA1_train_l_20.txt") # DCA1_train_l_5 DCA1_train_l_10
    # parser.add_argument("--t_un_txt_path", "--train_un_txt_path", default="./data/DCA1/DCA1_train_u_80.txt") # DCA1_train_u_95 DCA1_train_u_90 
