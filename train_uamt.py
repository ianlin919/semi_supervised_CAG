...
def train(fabric, model_, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    model, model_ema = model_
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), un_w in zip(cycle(train_loader_l), train_loader_u):
            ...
            """
            1. Add noisy on unlabeled data
            """
            noise = torch.clamp(torch.randn_like(un_w) * 0.1, -0.2, 0.2).to(fabric.device)
            ema_inputs = un_w + noise
            """
            1.1 teacher preds
            """
            imgs_all = torch.cat((imgs, un_w))
            output = model(imgs_all)
            out_l, out_w = output.split([imgs.shape[0], un_w.shape[0]])
            del imgs_all, output
            """
            1.2 student pred
            """
            ...
            T = 8
            _, _, w, h = un_w.shape
            un_w_r = un_w.repeat(2, 1, 1, 1)
            stride = un_w_r.shape[0] // 2
            preds = torch.zeros([stride * T, 1, w, h]).to(fabric.device)
            for i in range(T//2):
                ema_inputs = un_w_r + torch.clamp(torch.randn_like(un_w_r) * 0.1, -0.2, 0.2).to(fabric.device)
                with torch.no_grad():
                    preds[2 * stride * i:2 * stride * (i + 1)] = model_ema(ema_inputs)
            preds = torch.sigmoid(preds)
            preds = preds.reshape(T, stride, 1, w, h)
            preds = torch.mean(preds, dim=0)
            uncertainty = -1.0 * torch.sum(preds*torch.log(preds + 1e-6), dim=1, keepdim=True)
            """
            3. Supervised Loss
            """
            loss_su = loss_l(out_l, labels)
            su_loss = su_loss + (loss_su.detach().clone().item() / imgs.shape[0])
            """
            4. Unsupervised Loss
            """
            cons_weight = get_current_consistency_weight(iter_num)
            loss_un = loss_u(torch.sigmoid(out_w), torch.sigmoid(ema_output).detach())
            un_loss = un_loss + (loss_un.detach().clone().item() / un_batch_size)
            
            threshold = (0.75+0.25*cons_weight)*np.log(2)
            mask = (uncertainty < threshold).float()
            consistency_loss = torch.sum( mask*loss_un)/(2*torch.sum(mask)+1e-16)
            loss_ = loss_su + cons_weight * consistency_loss
            ...
            5. EMA model
            """
            model, model_ema = update_ema_variables(model, model_ema, 0.99, iter_num)
...
def main(args):
    ...
    """
    model = create_model()
    model_ema = create_model(norm=False ,ema=True)
    print("=" * 50)
    print("teacher model parameters: {:.2f}M".format(sum(p.numel() for p in model.parameters())/1e6))
    print("student model parameters: {:.2f}M".format(sum(p.numel() for p in model_ema.parameters())/1e6))
    print("=" * 50)
    loss = DiceBCELoss()
    loss_un = nn.MSELoss()
    ...
    model_ema = fabric.setup(model_ema)
    """
    ...
    for epoch in range(args.e):
        train_history = train(fabric, [model, model_ema], [train_loader, train_loader_u], 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/F2/20_500/UAMT"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/UAMT"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/UAMT"))
    # parser.add_argument("--t_txt_path", "--train_txt_path", default="./data/DCA1/DCA1_train_l_30.txt") # DCA1_train_l_5 DCA1_train_l_10 DCA1_train_l_20
    # parser.add_argument("--t_un_txt_path", "--train_un_txt_path", default="./data/DCA1/DCA1_train_u_70.txt") # DCA1_train_u_95 DCA1_train_u_90 DCA1_train_u_80
