...
class CMUNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=1, l=7, k=7):
        ...
        self.ConvMixer = ConvMixerBlock(dim=filters[4], depth=l, k=k)
        # Decoder
        self.Up5 = up_conv(ch_in=filters[4], ch_out=filters[3])
        self.Up_conv5 = conv_block(ch_in=filters[3] * 2, ch_out=filters[3])
        self.Up4 = up_conv(ch_in=filters[3], ch_out=filters[2])
        self.Up_conv4 = conv_block(ch_in=filters[2] * 2, ch_out=filters[2])
        self.Up3 = up_conv(ch_in=filters[2], ch_out=filters[1])
        self.Up_conv3 = conv_block(ch_in=filters[1] * 2, ch_out=filters[1])
        self.Up2 = up_conv(ch_in=filters[1], ch_out=filters[0])
        self.Up_conv2 = conv_block(ch_in=filters[0] * 2, ch_out=filters[0])
        self.Conv_1x1 = nn.Conv2d(filters[0], output_ch, kernel_size=1, stride=1, padding=0)
        # Skip-connection
        self.msag4 = MSAG(filters[3])
        self.msag3 = MSAG(filters[2])
        self.msag2 = MSAG(filters[1])
        self.msag1 = MSAG(filters[0])
