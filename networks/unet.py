...
class FReLU(nn.Module):
    """
    The implementation is borrowed from: https://github.com/shuuchen/frelu.pytorch/blob/master/frelu.py
    """
...
class SMU(nn.Module):
    '''
    The implementation is borrowed from: https://github.com/iFe1er/SMU_pytorch/blob/main/smu.py
    '''
...
class UNet_DTC(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.decoder(feature)
        if (self.training):
            output_tanh = self.tanh(output)
            return output, output_tanh
        return output
...
class UNet_UniMatch(nn.Module):
    ...
    def forward(self, x, need_fp=False):
        feature = self.encoder(x)
        main_seg = self.main_decoder(feature)
        if (self.training) and need_fp:
            aux_feature = [Dropout(i, p=0.5) for i in feature]
            aux_seg = self.main_decoder(aux_feature)
...
class UNet_URPC(nn.Module):
    ...
    def forward(self, x, need_dp=False):
        shape = x.shape[2:]
        feature = self.encoder(x)
        dp1_out_seg, dp2_out_seg, dp3_out_seg, dp4_out_seg = self.decoder(
            feature, shape)
        if (self.training) and need_dp:
