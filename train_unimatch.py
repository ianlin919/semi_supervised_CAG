...
## Data & Augmentation
from dataset import ImageDataSet_Train, ImageDataSet_Valid, ImageDataSet_Semi3
from transforms import get_train_augmentation
from transforms.RandAugment import RandAugment_best_2aug, RandAugment_best9aug
## Initial
ssl._create_default_https_context = ssl._create_unverified_context

def create_model(norm=True, ema=False):
    # net = smp.Unet("efficientnet-b6", in_channels=1, classes=1)
    net = net_factory('unet_unimatch',1,1)  # 'unet','enet','pnet'
    if norm:
...
def train(fabric, model, train_loader, loss, optimizer, scheduler, train_history, epoch, args):
    ...
    with tqdm(total=len(train_loader_u.dataset), desc="train ", unit="img",
        ...
        for (imgs, labels), (un_w, un_s1, un_s2) in zip(cycle(train_loader_l), train_loader_u):
            imgs, labels = imgs.float(), labels.float()
            un_w = un_w.float()
            un_s1, un_s2 = un_s1.float(), un_s2.float()
            ...
            """
            imgs_all = torch.cat((imgs, un_s1, un_s2))
            outputs = model(imgs_all)
            preds, pred_s1, pred_s2 = outputs.split([imgs.shape[0], un_s1.shape[0], un_s2.shape[0]])
            pred_w, pred_w_fp = model(un_w, need_fp=True)
            """
            1.1 Supervised Loss
            """
            ...
            mask = (torch.sigmoid(pred_w.detach()).ge(0.95)).to(dtype=torch.float32)
            mask1 = (torch.sigmoid(pred_w_fp.detach()).ge(0.95)).to(dtype=torch.float32)
            mask2 = (torch.sigmoid(pred_s1.detach()).ge(0.95)).to(dtype=torch.float32)
            mask3 = (torch.sigmoid(pred_s2.detach()).ge(0.95)).to(dtype=torch.float32)
            
            cons_loss1 = loss_u(pred_w_fp.sigmoid()*mask1, pred_w.detach().sigmoid()*mask)
            cons_loss2 = loss_u(pred_s1.sigmoid()*mask2, pred_w.detach().sigmoid()*mask)
            cons_loss3 = loss_u(pred_s2.sigmoid()*mask3, pred_w.detach().sigmoid()*mask)
            cons_loss = (cons_loss1*0.25 + cons_loss2*0.25 + cons_loss3*0.5)
            un_loss = un_loss + (cons_loss.detach().clone().item() / un_batch_size)
            
            loss_ = (loss_su + cons_loss) / 2.0
...
def main(args):
    ...
    print("=" * 50)
    loss = DiceBCELoss()
    from monai.losses.dice import DiceLoss as monai_dl
    loss_un = monai_dl(sigmoid=True)
    ...
    valid_dataset = ImageDataSet_Valid(
        ...
        size=args.size,
        sort=True)
    train_dataset_u = ImageDataSet_Semi3(
        un_txtPath=args.t_un_txt_path,
        img_dir=args.i_dir,
        size=args.size,
        transform_weak=weak_aug(),
        transform=RandAugment_best9aug(1, 20), # RandAugment_best_2aug(1, 20) RandAugment_best9aug(1, 20)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/CAG/1123/20_500/UniMatch"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/STARE/F6/18_377/UniMatch"))
    ...
    # parser.add_argument("--s_dir", "--save_dir", default=Path("./logs/DCA1/30_70/UniMatch"))
